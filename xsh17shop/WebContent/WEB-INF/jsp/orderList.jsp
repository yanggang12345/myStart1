...
<div class="container cart">
		...
		<div class="span24">
				...
				<table>
					...
					<tr>
						<th colspan="5">订单编号： <s:property value="#order.oid"/>
							订单状态
							<s:if test="#order.state ==1"><a href="${ pageContext.request.contextPath }/order_findByOid?oid=<s:property value="#order.oid"/>">未付款</a></s:if>
							<s:if test="#order.state ==2"><a href="#">已付款</a></s:if>
							<s:if test="#order.state ==3"><a href="${ pageContext.request.contextPath }/order_updateState?oid=<s:property value="#order.oid"/>">确认收货</a></s:if>
							<s:if test="#order.state ==4"><a href="#">交易完成</a></s:if>
						</th>
					</tr>
					<tr>
						<th>图片</th>
					...
					<s:iterator var="orderItem" value="#order.orderItems">
						<tr>
							...
							<td width="140">
								<span class="#orderItem.subtotal">￥<s:property value="#order.total"/>元</span>
							</td>
							<td>
								<input type="hidden" name="oid" value="<s:property value="#order.oid"/>"/>
								<input type="hidden" name="itemId" value="<s:property value="#orderItem.itemid"/>"/>
								<a href="${ pageContext.request.contextPath }/order_delete.action?oid=<s:property value="#order.oid"/>&&itemId=<s:property value="#orderItem.itemid"/>" class="delete">删除</a>
							</td>
						</tr>
						</s:iterator>
					</s:iterator>
			...
			<span>第 <s:property value="pageBean.page"/>/<s:property value="pageBean.totalPage=pageBean.totalPage==0?1:pageBean.totalPage"/> 页</span>
