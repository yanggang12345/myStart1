...
<script>
	...
	/* function checkUsername(){ */
		...
		xhr.onreadystatechange = function(){
			if(xhr.readyState == 4){
				if(xhr.status == 200){
					 document.getElementById("span1").innerHTML = xhr.response;
				}
			}
		}
		// 3.打开连接
...
$(function(){
	...
	$('#aCode1').click(function(){
		var phone=document.getElementById("phone").value;
		var url="${pageContext.request.contextPath}/user_codeActive.action?time="+new Date().getTime()+"&phone="+phone;
		$.post(url,function(result){
			console.log(phone);
		});
	});
	$('#aCode').blur(function() {
		var code=document.getElementById("aCode").value;
		var url="${pageContext.request.contextPath}/user_checkCodeActive.action?time="+new Date().getTime()+"&code="+code;
		$.post(url,function(result){
			if(result=="1"){
				$('#span2').text('验证码错误或者电话不存在');
			}else{
				$('#span2').text('可以使用');
			}
		});
	...
	/* $('#submit').click(function(){
		$.post("${ pageContext.request.contextPath }/user_regist.action",function(result){
			console.log(result);
			});
	}); */
});
							...
							<tr>
								...
								<td>
									...
									<tr>
										...
										<td>
												<input type="text" id="aCode" class="text" />
												<span id="span2"></span>
												<input type="button" id="aCode1"  class="submit" value="发送短信"/>
										</td>
									</tr>
